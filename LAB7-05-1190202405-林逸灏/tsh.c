...
int builtin_cmd(char **argv) 
{
	if(!strcmp(argv[0],"quit")){    //内置命令quit，退出tsh
        exit(0);
    }
	if(!strcmp(argv[0],"&")){//忽略单独的&，返回1
		return 1;
	}
    if(!strcmp(argv[0],"bg") || (!strcmp(argv[0],"fg"))) {  //内置命令fg或bg，调用do_fgbg，返回1
        do_bgfg(argv);
        return 1;
    }
    if(!strcmp(argv[0],"jobs")) {     //内置命令jobs，列出所有任务，调用 listjobs 即可，返回1
        listjobs(jobs);
        return 1;
    }
...
void waitfg(pid_t pid)
{
	while (fgpid(jobs) == pid) { //判断前台进程的pid是否为输入的pid
		sleep(1);  //睡眠一秒
	}
    return;
}
...
void sigint_handler(int sig) 
{
    ...
    kill(-pid, SIGINT);       //向处于前台正在运行的进程的进程组发送SIGINT
    errno = olderrno;
    return;
}
